...
def predict(data_points, model, batch_size):
    ...
    for batch in dataloader:
        ...
        model.predict(batch) # predict on the batch of sentences
...
def scorer_evaluate(reference, predictions):
    ...
    for reference, prediction in zip(reference, predictions):
        scores.merge(Scorer(Scorer.create_mentions(reference.get_labels()), Scorer.create_mentions(prediction.get_labels())))
    return scores
